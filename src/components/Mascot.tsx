...
interface Props {
  ...
  className?: string;
  /** If true, ignore equipped items and only use props (e.g., for previews) */
  preview?: boolean;
}

/**
 * Carraco — owl mascot. When `hat`/`glasses` are omitted (and `preview` is false),
 * automatically renders the currently-equipped cosmetics from the user's inventory.
 */
const Mascot = ({ size = 120, hat, glasses, mood = "happy", className = "", preview = false }: Props) => {
  const { equipped } = useEquippedMascot();
  const resolvedHat = hat ?? (preview ? "none" : equipped.hat);
  const resolvedGlasses = glasses ?? (preview ? "none" : equipped.glasses);
